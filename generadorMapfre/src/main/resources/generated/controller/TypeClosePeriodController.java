...
public class TypeClosePeriodController implements ITypeClosePeriodController{
	...
	@Override
	public ResponseEntity<List<TypeClosePeriodBO>> get() {
		try {
			return ResponseEntity.ok().body(typeClosePeriodBL.getAll());
		} catch (Exception e) {
			log.error("TypeClosePeriodController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeClosePeriodBO> add(@Valid @RequestBody TypeClosePeriodBO input) {
    	try {
			TypeClosePeriodBO typeClosePeriodBo = closePeriodBL.add(input);
