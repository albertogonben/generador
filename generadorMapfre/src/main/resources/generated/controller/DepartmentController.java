...
public class DepartmentController implements IDepartmentController{
	...
	@Override
	public ResponseEntity<List<DepartmentBO>> get() {
		try {
			return ResponseEntity.ok().body(departmentBL.getAll());
		} catch (Exception e) {
			log.error("DepartmentController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<DepartmentBO> add(@Valid @RequestBody DepartmentBO input) {
    	try {
			DepartmentBO departmentBo = closePeriodBL.add(input);
