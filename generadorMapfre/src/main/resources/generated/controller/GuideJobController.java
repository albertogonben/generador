...
public class GuideJobController implements IGuideJobController{
	...
	@Override
	public ResponseEntity<List<GuideJobBO>> get() {
		try {
			return ResponseEntity.ok().body(guideJobBL.getAll());
		} catch (Exception e) {
			log.error("GuideJobController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<GuideJobBO> add(@Valid @RequestBody GuideJobBO input) {
    	try {
			GuideJobBO guideJobBo = closePeriodBL.add(input);
