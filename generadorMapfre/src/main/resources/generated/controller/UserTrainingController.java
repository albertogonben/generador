...
public class UserTrainingController implements IUserTrainingController{
	...
	@Override
	public ResponseEntity<List<UserTrainingBO>> get() {
		try {
			return ResponseEntity.ok().body(userTrainingBL.getAll());
		} catch (Exception e) {
			log.error("UserTrainingController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<UserTrainingBO> add(@Valid @RequestBody UserTrainingBO input) {
    	try {
			UserTrainingBO userTrainingBo = closePeriodBL.add(input);
