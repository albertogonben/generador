...
public class RegionController implements IRegionController{
	...
	@Override
	public ResponseEntity<List<RegionBO>> get() {
		try {
			return ResponseEntity.ok().body(regionBL.getAll());
		} catch (Exception e) {
			log.error("RegionController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<RegionBO> add(@Valid @RequestBody RegionBO input) {
    	try {
			RegionBO regionBo = closePeriodBL.add(input);
