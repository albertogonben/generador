...
public class TypeTraceController implements ITypeTraceController{
	...
	@Override
	public ResponseEntity<List<TypeTraceBO>> get() {
		try {
			return ResponseEntity.ok().body(typeTraceBL.getAll());
		} catch (Exception e) {
			log.error("TypeTraceController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeTraceBO> add(@Valid @RequestBody TypeTraceBO input) {
    	try {
			TypeTraceBO typeTraceBo = closePeriodBL.add(input);
