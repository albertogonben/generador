...
public class ClosePeriodController implements IClosePeriodController{
	...
	@Override
	public ResponseEntity<List<ClosePeriodBO>> get() {
		try {
			return ResponseEntity.ok().body(closePeriodBL.getAll());
		} catch (Exception e) {
			log.error("ClosePeriodController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<ClosePeriodBO> add(@Valid @RequestBody ClosePeriodBO input) {
    	try {
