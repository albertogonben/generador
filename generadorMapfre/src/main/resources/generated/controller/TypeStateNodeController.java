...
@Slf4j
@RestController
public class TypeStateNodeController implements ITypeStateNodeController{

	private ITypeStateNodeBL typeStateNodeBL;
	
	@Autowired
	public TypeStateNodeController(ITypeStateNodeBL typeStateNodeBL) {
		this.typeStateNodeBL = typeStateNodeBL;
	}
	
	@Override
	public ResponseEntity<List<TypeStateNodeBO>> get() {
		try {
			return ResponseEntity.ok().body(typeStateNodeBL.getAll());
		} catch (Exception e) {
			log.error("TypeStateNodeController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeStateNodeBO> add(@Valid @RequestBody TypeStateNodeBO input) {
    	try {
			TypeStateNodeBO typeStateNodeBo = closePeriodBL.add(input);
