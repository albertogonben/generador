...
public class TaskDocumentController implements ITaskDocumentController{
	...
	@Override
	public ResponseEntity<List<TaskDocumentBO>> get() {
		try {
			return ResponseEntity.ok().body(taskDocumentBL.getAll());
		} catch (Exception e) {
			log.error("TaskDocumentController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TaskDocumentBO> add(@Valid @RequestBody TaskDocumentBO input) {
    	try {
			TaskDocumentBO taskDocumentBo = closePeriodBL.add(input);
