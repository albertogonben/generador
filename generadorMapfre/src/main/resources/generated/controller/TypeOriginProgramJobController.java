...
public class TypeOriginProgramJobController implements ITypeOriginProgramJobController{
	...
	@Override
	public ResponseEntity<List<TypeOriginProgramJobBO>> get() {
		try {
			return ResponseEntity.ok().body(typeOriginProgramJobBL.getAll());
		} catch (Exception e) {
			log.error("TypeOriginProgramJobController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeOriginProgramJobBO> add(@Valid @RequestBody TypeOriginProgramJobBO input) {
    	try {
			TypeOriginProgramJobBO typeOriginProgramJobBo = closePeriodBL.add(input);
