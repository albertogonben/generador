...
@Slf4j
@RestController
public class UserAlarmController implements IUserAlarmController{

	private IUserAlarmBL userAlarmBL;
	
	@Autowired
	public UserAlarmController(IUserAlarmBL userAlarmBL) {
		this.userAlarmBL = userAlarmBL;
	}
	
	@Override
	public ResponseEntity<List<UserAlarmBO>> get() {
		try {
			return ResponseEntity.ok().body(userAlarmBL.getAll());
		} catch (Exception e) {
			log.error("UserAlarmController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<UserAlarmBO> add(@Valid @RequestBody UserAlarmBO input) {
    	try {
			UserAlarmBO userAlarmBo = closePeriodBL.add(input);
