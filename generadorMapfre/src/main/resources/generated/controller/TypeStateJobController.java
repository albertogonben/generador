...
public class TypeStateJobController implements ITypeStateJobController{
	...
	@Override
	public ResponseEntity<List<TypeStateJobBO>> get() {
		try {
			return ResponseEntity.ok().body(typeStateJobBL.getAll());
		} catch (Exception e) {
			log.error("TypeStateJobController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeStateJobBO> add(@Valid @RequestBody TypeStateJobBO input) {
    	try {
			TypeStateJobBO typeStateJobBo = closePeriodBL.add(input);
