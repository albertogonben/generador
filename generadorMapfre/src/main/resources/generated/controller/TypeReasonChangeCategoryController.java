...
public class TypeReasonChangeCategoryController implements ITypeReasonChangeCategoryController{
	...
	@Override
	public ResponseEntity<List<TypeReasonChangeCategoryBO>> get() {
		try {
			return ResponseEntity.ok().body(typeReasonChangeCategoryBL.getAll());
		} catch (Exception e) {
			log.error("TypeReasonChangeCategoryController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeReasonChangeCategoryBO> add(@Valid @RequestBody TypeReasonChangeCategoryBO input) {
    	try {
			TypeReasonChangeCategoryBO typeReasonChangeCategoryBo = closePeriodBL.add(input);
