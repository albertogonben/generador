...
public class TagController implements ITagController{
	...
	@Override
	public ResponseEntity<List<TagBO>> get() {
		try {
			return ResponseEntity.ok().body(tagBL.getAll());
		} catch (Exception e) {
			log.error("TagController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TagBO> add(@Valid @RequestBody TagBO input) {
    	try {
			TagBO tagBo = closePeriodBL.add(input);
