...
@Slf4j
@RestController
public class TypeStateIdeaController implements ITypeStateIdeaController{

	private ITypeStateIdeaBL typeStateIdeaBL;
	
	@Autowired
	public TypeStateIdeaController(ITypeStateIdeaBL typeStateIdeaBL) {
		this.typeStateIdeaBL = typeStateIdeaBL;
	}
	
	@Override
	public ResponseEntity<List<TypeStateIdeaBO>> get() {
		try {
			return ResponseEntity.ok().body(typeStateIdeaBL.getAll());
		} catch (Exception e) {
			log.error("TypeStateIdeaController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeStateIdeaBO> add(@Valid @RequestBody TypeStateIdeaBO input) {
    	try {
			TypeStateIdeaBO typeStateIdeaBo = closePeriodBL.add(input);
