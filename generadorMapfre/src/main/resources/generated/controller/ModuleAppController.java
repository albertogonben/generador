...
public class ModuleAppController implements IModuleAppController{
	...
	@Override
	public ResponseEntity<List<ModuleAppBO>> get() {
		try {
			return ResponseEntity.ok().body(moduleAppBL.getAll());
		} catch (Exception e) {
			log.error("ModuleAppController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<ModuleAppBO> add(@Valid @RequestBody ModuleAppBO input) {
    	try {
			ModuleAppBO moduleAppBo = closePeriodBL.add(input);
