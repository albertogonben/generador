...
public class TypeLevelLanguageController implements ITypeLevelLanguageController{
	...
	@Override
	public ResponseEntity<List<TypeLevelLanguageBO>> get() {
		try {
			return ResponseEntity.ok().body(typeLevelLanguageBL.getAll());
		} catch (Exception e) {
			log.error("TypeLevelLanguageController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeLevelLanguageBO> add(@Valid @RequestBody TypeLevelLanguageBO input) {
    	try {
			TypeLevelLanguageBO typeLevelLanguageBo = closePeriodBL.add(input);
