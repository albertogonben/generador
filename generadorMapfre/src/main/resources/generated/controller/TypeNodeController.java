...
public class TypeNodeController implements ITypeNodeController{
	...
	@Override
	public ResponseEntity<List<TypeNodeBO>> get() {
		try {
			return ResponseEntity.ok().body(typeNodeBL.getAll());
		} catch (Exception e) {
			log.error("TypeNodeController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeNodeBO> add(@Valid @RequestBody TypeNodeBO input) {
    	try {
			TypeNodeBO typeNodeBo = closePeriodBL.add(input);
