...
public class LanguageController implements ILanguageController{
	...
	@Override
	public ResponseEntity<List<LanguageBO>> get() {
		try {
			return ResponseEntity.ok().body(languageBL.getAll());
		} catch (Exception e) {
			log.error("LanguageController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<LanguageBO> add(@Valid @RequestBody LanguageBO input) {
    	try {
			LanguageBO languageBo = closePeriodBL.add(input);
