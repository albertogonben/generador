...
@Slf4j
@RestController
public class JobOriginController implements IJobOriginController{

	private IJobOriginBL jobOriginBL;
	
	@Autowired
	public JobOriginController(IJobOriginBL jobOriginBL) {
		this.jobOriginBL = jobOriginBL;
	}
	
	@Override
	public ResponseEntity<List<JobOriginBO>> get() {
		try {
			return ResponseEntity.ok().body(jobOriginBL.getAll());
		} catch (Exception e) {
			log.error("JobOriginController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<JobOriginBO> add(@Valid @RequestBody JobOriginBO input) {
    	try {
			JobOriginBO jobOriginBo = closePeriodBL.add(input);
