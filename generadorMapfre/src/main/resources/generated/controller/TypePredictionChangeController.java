...
public class TypePredictionChangeController implements ITypePredictionChangeController{
	...
	@Override
	public ResponseEntity<List<TypePredictionChangeBO>> get() {
		try {
			return ResponseEntity.ok().body(typePredictionChangeBL.getAll());
		} catch (Exception e) {
			log.error("TypePredictionChangeController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypePredictionChangeBO> add(@Valid @RequestBody TypePredictionChangeBO input) {
    	try {
			TypePredictionChangeBO typePredictionChangeBo = closePeriodBL.add(input);
