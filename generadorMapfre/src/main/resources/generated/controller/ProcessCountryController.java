...
public class ProcessCountryController implements IProcessCountryController{
	...
	@Override
	public ResponseEntity<List<ProcessCountryBO>> get() {
		try {
			return ResponseEntity.ok().body(processCountryBL.getAll());
		} catch (Exception e) {
			log.error("ProcessCountryController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<ProcessCountryBO> add(@Valid @RequestBody ProcessCountryBO input) {
    	try {
			ProcessCountryBO processCountryBo = closePeriodBL.add(input);
