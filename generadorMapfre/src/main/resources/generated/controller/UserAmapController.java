...
public class UserAmapController implements IUserAmapController{
	...
	@Override
	public ResponseEntity<List<UserAmapBO>> get() {
		try {
			return ResponseEntity.ok().body(userAmapBL.getAll());
		} catch (Exception e) {
			log.error("UserAmapController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<UserAmapBO> add(@Valid @RequestBody UserAmapBO input) {
    	try {
			UserAmapBO userAmapBo = closePeriodBL.add(input);
