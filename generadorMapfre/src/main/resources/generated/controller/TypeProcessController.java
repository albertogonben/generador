...
@Slf4j
@RestController
public class TypeProcessController implements ITypeProcessController{

	private ITypeProcessBL typeProcessBL;
	
	@Autowired
	public TypeProcessController(ITypeProcessBL typeProcessBL) {
		this.typeProcessBL = typeProcessBL;
	}
	
	@Override
	public ResponseEntity<List<TypeProcessBO>> get() {
		try {
			return ResponseEntity.ok().body(typeProcessBL.getAll());
		} catch (Exception e) {
			log.error("TypeProcessController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeProcessBO> add(@Valid @RequestBody TypeProcessBO input) {
    	try {
			TypeProcessBO typeProcessBo = closePeriodBL.add(input);
