...
public class TypeOrganizationalStructureController implements ITypeOrganizationalStructureController{
	...
	@Override
	public ResponseEntity<List<TypeOrganizationalStructureBO>> get() {
		try {
			return ResponseEntity.ok().body(typeOrganizationalStructureBL.getAll());
		} catch (Exception e) {
			log.error("TypeOrganizationalStructureController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeOrganizationalStructureBO> add(@Valid @RequestBody TypeOrganizationalStructureBO input) {
    	try {
			TypeOrganizationalStructureBO typeOrganizationalStructureBo = closePeriodBL.add(input);
