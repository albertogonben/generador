...
public class TypeCriquiteAlarmController implements ITypeCriquiteAlarmController{
	...
	@Override
	public ResponseEntity<List<TypeCriquiteAlarmBO>> get() {
		try {
			return ResponseEntity.ok().body(typeCriquiteAlarmBL.getAll());
		} catch (Exception e) {
			log.error("TypeCriquiteAlarmController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeCriquiteAlarmBO> add(@Valid @RequestBody TypeCriquiteAlarmBO input) {
    	try {
			TypeCriquiteAlarmBO typeCriquiteAlarmBo = closePeriodBL.add(input);
