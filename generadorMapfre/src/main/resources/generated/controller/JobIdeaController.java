...
public class JobIdeaController implements IJobIdeaController{
	...
	@Override
	public ResponseEntity<List<JobIdeaBO>> get() {
		try {
			return ResponseEntity.ok().body(jobIdeaBL.getAll());
		} catch (Exception e) {
			log.error("JobIdeaController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<JobIdeaBO> add(@Valid @RequestBody JobIdeaBO input) {
    	try {
			JobIdeaBO jobIdeaBo = closePeriodBL.add(input);
