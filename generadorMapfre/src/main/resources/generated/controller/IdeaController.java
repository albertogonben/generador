...
public class IdeaController implements IIdeaController{
	...
	private IIdeaBL ideaBL;
	
	@Autowired
	public IdeaController(IIdeaBL ideaBL) {
		this.ideaBL = ideaBL;
	}
	
	@Override
	public ResponseEntity<List<IdeaBO>> get() {
		try {
			return ResponseEntity.ok().body(ideaBL.getAll());
		} catch (Exception e) {
			log.error("IdeaController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<IdeaBO> add(@Valid @RequestBody IdeaBO input) {
    	try {
			IdeaBO ideaBo = closePeriodBL.add(input);
