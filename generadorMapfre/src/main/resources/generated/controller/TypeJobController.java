...
@Slf4j
@RestController
public class TypeJobController implements ITypeJobController{

	private ITypeJobBL typeJobBL;
	
	@Autowired
	public TypeJobController(ITypeJobBL typeJobBL) {
		this.typeJobBL = typeJobBL;
	}
	
	@Override
	public ResponseEntity<List<TypeJobBO>> get() {
		try {
			return ResponseEntity.ok().body(typeJobBL.getAll());
		} catch (Exception e) {
			log.error("TypeJobController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeJobBO> add(@Valid @RequestBody TypeJobBO input) {
    	try {
			TypeJobBO typeJobBo = closePeriodBL.add(input);
