...
@Slf4j
@RestController
public class JobTagController implements IJobTagController{

	private IJobTagBL jobTagBL;
	
	@Autowired
	public JobTagController(IJobTagBL jobTagBL) {
		this.jobTagBL = jobTagBL;
	}
	
	@Override
	public ResponseEntity<List<JobTagBO>> get() {
		try {
			return ResponseEntity.ok().body(jobTagBL.getAll());
		} catch (Exception e) {
			log.error("JobTagController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<JobTagBO> add(@Valid @RequestBody JobTagBO input) {
    	try {
			JobTagBO jobTagBo = closePeriodBL.add(input);
