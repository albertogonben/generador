...
public class UserCategoryController implements IUserCategoryController{
	...
	@Override
	public ResponseEntity<List<UserCategoryBO>> get() {
		try {
			return ResponseEntity.ok().body(userCategoryBL.getAll());
		} catch (Exception e) {
			log.error("UserCategoryController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<UserCategoryBO> add(@Valid @RequestBody UserCategoryBO input) {
    	try {
			UserCategoryBO userCategoryBo = closePeriodBL.add(input);
