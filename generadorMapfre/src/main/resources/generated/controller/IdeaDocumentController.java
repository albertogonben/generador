...
public class IdeaDocumentController implements IIdeaDocumentController{
	...
	@Override
	public ResponseEntity<List<IdeaDocumentBO>> get() {
		try {
			return ResponseEntity.ok().body(ideaDocumentBL.getAll());
		} catch (Exception e) {
			log.error("IdeaDocumentController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<IdeaDocumentBO> add(@Valid @RequestBody IdeaDocumentBO input) {
    	try {
			IdeaDocumentBO ideaDocumentBo = closePeriodBL.add(input);
