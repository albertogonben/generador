...
@Slf4j
@RestController
public class ProgramJobController implements IProgramJobController{

	private IProgramJobBL programJobBL;
	
	@Autowired
	public ProgramJobController(IProgramJobBL programJobBL) {
		this.programJobBL = programJobBL;
	}
	
	@Override
	public ResponseEntity<List<ProgramJobBO>> get() {
		try {
			return ResponseEntity.ok().body(programJobBL.getAll());
		} catch (Exception e) {
			log.error("ProgramJobController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<ProgramJobBO> add(@Valid @RequestBody ProgramJobBO input) {
    	try {
			ProgramJobBO programJobBo = closePeriodBL.add(input);
