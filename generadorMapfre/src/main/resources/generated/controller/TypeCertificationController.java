...
public class TypeCertificationController implements ITypeCertificationController{
	...
	@Override
	public ResponseEntity<List<TypeCertificationBO>> get() {
		try {
			return ResponseEntity.ok().body(typeCertificationBL.getAll());
		} catch (Exception e) {
			log.error("TypeCertificationController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeCertificationBO> add(@Valid @RequestBody TypeCertificationBO input) {
    	try {
			TypeCertificationBO typeCertificationBo = closePeriodBL.add(input);
