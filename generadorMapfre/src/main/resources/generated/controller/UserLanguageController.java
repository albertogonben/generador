...
public class UserLanguageController implements IUserLanguageController{
	...
	@Override
	public ResponseEntity<List<UserLanguageBO>> get() {
		try {
			return ResponseEntity.ok().body(userLanguageBL.getAll());
		} catch (Exception e) {
			log.error("UserLanguageController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<UserLanguageBO> add(@Valid @RequestBody UserLanguageBO input) {
    	try {
			UserLanguageBO userLanguageBo = closePeriodBL.add(input);
