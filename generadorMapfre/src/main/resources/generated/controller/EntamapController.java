...
public class EntamapController implements IEntamapController{
	...
	@Override
	public ResponseEntity<List<EntamapBO>> get() {
		try {
			return ResponseEntity.ok().body(entamapBL.getAll());
		} catch (Exception e) {
			log.error("EntamapController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<EntamapBO> add(@Valid @RequestBody EntamapBO input) {
    	try {
			EntamapBO entamapBo = closePeriodBL.add(input);
