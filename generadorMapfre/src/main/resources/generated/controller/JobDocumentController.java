...
public class JobDocumentController implements IJobDocumentController{
	...
	@Override
	public ResponseEntity<List<JobDocumentBO>> get() {
		try {
			return ResponseEntity.ok().body(jobDocumentBL.getAll());
		} catch (Exception e) {
			log.error("JobDocumentController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<JobDocumentBO> add(@Valid @RequestBody JobDocumentBO input) {
    	try {
			JobDocumentBO jobDocumentBo = closePeriodBL.add(input);
