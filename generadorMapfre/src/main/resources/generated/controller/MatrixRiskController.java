...
public class MatrixRiskController implements IMatrixRiskController{
	...
	@Override
	public ResponseEntity<List<MatrixRiskBO>> get() {
		try {
			return ResponseEntity.ok().body(matrixRiskBL.getAll());
		} catch (Exception e) {
			log.error("MatrixRiskController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<MatrixRiskBO> add(@Valid @RequestBody MatrixRiskBO input) {
    	try {
			MatrixRiskBO matrixRiskBo = closePeriodBL.add(input);
