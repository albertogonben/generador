...
public class JobLogController implements IJobLogController{
	...
	private IJobLogBL jobLogBL;
	
	@Autowired
	public JobLogController(IJobLogBL jobLogBL) {
		this.jobLogBL = jobLogBL;
	}
	
	@Override
	public ResponseEntity<List<JobLogBO>> get() {
		try {
			return ResponseEntity.ok().body(jobLogBL.getAll());
		} catch (Exception e) {
			log.error("JobLogController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<JobLogBO> add(@Valid @RequestBody JobLogBO input) {
    	try {
			JobLogBO jobLogBo = closePeriodBL.add(input);
