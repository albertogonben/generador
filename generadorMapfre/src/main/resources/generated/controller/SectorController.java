...
public class SectorController implements ISectorController{
	...
	@Override
	public ResponseEntity<List<SectorBO>> get() {
		try {
			return ResponseEntity.ok().body(sectorBL.getAll());
		} catch (Exception e) {
			log.error("SectorController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<SectorBO> add(@Valid @RequestBody SectorBO input) {
    	try {
			SectorBO sectorBo = closePeriodBL.add(input);
