...
public class TypeCategoryAlarmController implements ITypeCategoryAlarmController{
	...
	@Override
	public ResponseEntity<List<TypeCategoryAlarmBO>> get() {
		try {
			return ResponseEntity.ok().body(typeCategoryAlarmBL.getAll());
		} catch (Exception e) {
			log.error("TypeCategoryAlarmController:get", e);
			return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).build();
		}
	}

    @Override
    public ResponseEntity<TypeCategoryAlarmBO> add(@Valid @RequestBody TypeCategoryAlarmBO input) {
    	try {
			TypeCategoryAlarmBO typeCategoryAlarmBo = closePeriodBL.add(input);
